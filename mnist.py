...
from keras.layers import Input, Dense, MaxPool2D, UpSampling2D, Conv2D, concatenate, Flatten, Activation, Reshape
...
class Discriminator(object):
    ...
    def __init__(self,height,width,RGB,condition_dim):
        ...
        discriminator_input_1 = Input(shape=(height,width,RGB,),name='d_1')
        ...
        discriminator_input = concatenate([discriminator_input_1, _discriminator_input_2])
...
class ConditionalDCGAN(object):
    ...
    def __init__(self,latent_dim,height,width,RGB,condition_dim):
        ...
        dcgan_output = self._discriminator(self._generator([dcgan_input_1,dcgan_input_2]))
...
def train(latent_dim,height,width,RGB,num_class,path):
    ...
    for i in e_range:
        ...
        if (i+1)%1 == 0:
            ...
            dcgan.save_weights(os.path.join(path,'gan' + '_epoch_' + str(i+1) + '.h5'))
            ...
            img.save(os.path.join(path,'gen_during'+ str(i+1) + '_' + str(condition) + '.png'))
