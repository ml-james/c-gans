...
class Discriminator(object):
    ...
    def __init__(self,condition_dim):
        ...
        discriminator_input = concatenate([discriminator_input_1, discriminator_input_2])
...
class ConditionalGAN(object):
    ...
    def train_dis(self,_image,condition,noise,batch_size):
       ...
       d_loss = self._discriminator_.train_on_batch([_image,condition,noise],[y_true,y_false,y_dummy])[0]
...
def train(latent_dim,condition_dim,epochs,path):
    ...
    for epoch in range(epochs):
        ...
        if epoch%1 == 0:
            ...
            plt.hist(diff_plot[:,0],bins,histtype='step',color='red',label='momentum')
            plt.hist(diff_plot[:,1],bins,histtype='step',color='green',label='eta')
            plt.hist(diff_plot[:,2],bins,histtype='step',color='yellow',label='phi')
            ...
            plt.hist(_condition[:,0],bins,histtype='step',color='red',label='condition')
            plt.xlabel("Momentum / GeV")
            plt.ylabel("Number")
            plt.legend(loc="upper right")
            ...
            plt.hist(_condition[:,1],bins,histtype='step',color='red',label='condition')
            ...
            plt.hist(_condition[:,2],bins,histtype='step',color='red',label='condition')
