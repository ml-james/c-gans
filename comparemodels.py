...
def Tracking_Efficiency_Electron(events_array):
    ...
    output_ = np.zeros((events_array.shape[0],events_array.shape[1],events_array.shape[2]))
    ...
    for i in range(events_array.shape[0]):
        for j in range(events_array.shape[1]):
            if abs(events_array[i][j][0]) <= 0.1:
                ...
                output_[i][j][1] = 0.0
                output_[i][j][2] = 0.0
            if abs(events_array[i][j][0]) <= 1.5 and abs(events_array[i][j][0]) > 0.1 and abs(events_array[i][j][0]) <= 1.0:
                ...
                if a < 0.73:
                    ...
                    output_[i][j][1] = 0.0
                    output_[i][j][2] = 0.0
            if abs(events_array[i][j][1]) <= 1.5 and abs(events_array[i][j][0]) > 0.1 and abs(events_array[i][j][0]) <= 10:
                a = np.random.randint(0,1)
                if a < 0.95:
                    output_[i][j][0] = events_array[i][j][0]
                    output_[i][j][1] = events_array[i][j][1]
            ...
            if abs(events_array[i][j][1]) <= 1.5 and abs(events_array[i][j][0]) > 10:
                ...
                if a < 0.99:
                    ...
                    output_[i][j][1] = 0.0
                    output_[i][j][2] = 0.0
            if abs(events_array[i][j][1]) > 1.5 and abs(events_array[i][j][1]) <= 2.5 and abs(events_array[i][j][0]) > 0.1 and abs(events_array[i][j][0]) <= 1.0:
                a = np.random.randint(0,1)
                if a < 0.50:
                    output_[i][j][0] = events_array[i][j][0]
                    output_[i][j][1] = events_array[i][j][1]
                    ...
                    output_[i][j][1] = 0.0
                    output_[i][j][2] = 0.0
            if abs(events_array[i][j][1]) > 1.5 and abs(events_array[i][j][1]) <= 2.5 and abs(events_array[i][j][0]) > 0.1 and abs(events_array[i][j][0]) <= 10:
                a = np.random.randint(0,1)
                if a < 0.83:
                    output_[i][j][0] = events_array[i][j][0]
                    output_[i][j][1] = events_array[i][j][1]
                    ...
                    output_[i][j][1] = 0.0
                    output_[i][j][2] = 0.0
            if abs(events_array[i][j][1]) > 1.5 and abs(events_array[i][j][1]) <= 2.5 and abs(events_array[i][j][0]) > 10:
                a = np.random.randint(0,1)
                if a < 0.90:
                    output_[i][j][0] = events_array[i][j][0]
                    output_[i][j][1] = events_array[i][j][1]
    ...
    for i in g_range:
        for j in range(input_.shape[1]):
            if abs(input_[i][j][1]) <= 0.5 and input_[i][j][0] > 0.1:
                res_ = abs(input_[i][j][0]) * abs(input_[i][j][1]) * math.sqrt(0.03*0.03 + (input_[i][j][0]*input_[i][j][0] * (1.30e-3) * (1.30e-3)))
            if abs(input_[i][j][1]) > 0.5 and abs(input_[i][j][1]) <= 1.5 and input_[i][j][0] > 0.1:
                res_ = abs(input_[i][j][0]) * abs(input_[i][j][1]) * math.sqrt(0.05*0.05 + (input_[i][j][0]*input_[i][j][0] * (1.70e-3) * (1.70e-3)))
            if abs(input_[i][j][1]) > 1.5 and abs(input_[i][j][1]) <= 2.5 and input_[i][j][0] > 0.1:
                res_ = abs(input_[i][j][0]) * abs(input_[i][j][1]) * math.sqrt(0.15*0.15 + (input_[i][j][0]*input_[i][j][0] * (3.10e-3) * (3.10e-3)))
...
def Smear(input_,resolution):
    ...
    for i in g_range:
        for j in range(input_.shape[1]):
            Smear[i][j][0] = Momentum_Smearing_Log_Normal(input_[i][j][0],resolution[i][j][0]*input_[i][j][0])
...
def Run_Entire_Spike(events,p,eta,phi):
    ...
    Fake_ = Smear(Real_,Resolution_Trimming(Resolution(Tracking_Efficiency_Electron(Real_))))
...
def run_comparison(events,p,eta,phi):
    ...
    binsa = np.linspace(min(a[0,:,0]),max(a[0,:,0]),int(max((a[0,:,0])-min(a[0,:,0])))*2)
    binsb = np.linspace(min(b[0,:,0]),max(b[0,:,0]),int(max((b[0,:,0])-min(b[0,:,0])))*2)
    #bins=100
