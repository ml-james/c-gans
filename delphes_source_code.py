    ...
    for i in g_range:
        for j in range(len(eta)):
            for k in range(len(phi)):
                ...
                Fake_[l] = Smear(Real_[l],Resolution_Trimming(Resolution(Tracking_Efficiency_Electron(Real_[l]))))
...
def Run_Entire_Random():
    ...
    Fake = Smear(Real,Resolution_Trimming(Resolution(Tracking_Efficiency_Electron(Real))))
...
def plot(event1,event2,event3):
    ...
    bins = np.linspace(min(a[event1,event2,:,event3]),max(a[event1,0,:,event3]),50)
    plt.hist(a[event1,event2,:,event3],bins,histtype='step',color='green',label='delphes')
    plt.title("Transverse Momentum Distribution of \n Electrons Fired from an Electron Gun")
    plt.xlabel("Momentum / GeV")
    plt.ylabel("Number")
    plt.legend(loc='upper right')
