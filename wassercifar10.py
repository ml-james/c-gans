...
class Discriminator(object):
    def __init__(self, condition_dim):
        ...
        discriminator_input = concatenate(
            [discriminator_input_1, _discriminator_input_2], name="is_it_here"
        )

        x = Conv2D(64, (5, 5), padding="same")(discriminator_input)
        x = LeakyReLU()(x)
        x = Conv2D(
            128, (5, 5), kernel_initializer="he_normal", strides=[2, 2], padding="same"
        )(x)
        x = LeakyReLU()(x)
        x = Conv2D(
            128, (5, 5), kernel_initializer="he_normal", strides=[2, 2], padding="same"
...
class ConditionalGAN(object):
    ...
    def train_dis(self, image, condition, noise, batch_size):
        ...
        d_loss = self._discriminator_.train_on_batch(
            [image, condition, noise], [y_true, y_false, y_dummy]
        )[0]
...
def train(latent_dim, condition_dim, epochs, path):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 1 == 0:
            ...
            img.save(
                os.path.join(path, "g_d" + str(epoch + 1) + "_" + str(c + 1) + ".png")
            )
...
def predict(latent_dim, condition_dim, path_):
    ...
    model_ = Model(
        inputs=loaded_model.input, outputs=loaded_model.layers[2].get_output_at(1)
    )
    ...
    for i in range(n):
        ...
        img.save(
            os.path.join(
                path_,
                "g_after" + "_" + "con" + str(c + 1) + "_sample_" + str(i) + ".png",
            )
        )
